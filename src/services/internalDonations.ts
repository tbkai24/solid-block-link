...
export async function addInternalDonation(campaignId: string, name: string, amount: number, notes: string) {
  if (!supabase) throw new Error("Supabase is not configured.");
  const base = { campaign_id: campaignId, amount, notes };
  const attemptName = await supabase.from("campaign_internal_adjustments").insert({ ...base, name }).select("*").single();
...
export async function updateInternalDonation(id: string, name: string, amount: number, notes: string) {
  if (!supabase) throw new Error("Supabase is not configured.");
  const base = { amount, notes };
  const attemptName = await supabase.from("campaign_internal_adjustments").update({ ...base, name }).eq("id", id).select("*").single();
  if (!attemptName.error) return toInternalAdjustment(attemptName.data as RawInternalAdjustment);
  if (!attemptName.error.message.toLowerCase().includes("name")) throw attemptName.error;
