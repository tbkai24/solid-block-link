...
function getPublicRaisedFromSummary(summary: Awaited<ReturnType<typeof getDonationSummaryForCampaign>>) {
...
function getPublicDonorsFromSummary(summary: Awaited<ReturnType<typeof getDonationSummaryForCampaign>>) {
...
export function AdminMilestonesPanel() {
  ...
  async function refreshSnapshot(nextCampaign?: Partial<CampaignRow> | null, rows: CampaignMilestoneRow[] = milestoneRows) {
    const publicSummary = await getDonationSummaryForCampaign(rows.map((row) => ({
      milestoneId: row.id,
      title: row.title,
    ...
    const publicRaised = getPublicRaisedFromSummary(publicSummary);
    const internalRaised = Number(nextCampaign?.internal_amount ?? 0);
    const internalRes = nextCampaign?.id && supabase
      ? await supabase
    ...
    setSnapshot({
      ...
      donorCount: getPublicDonorsFromSummary(publicSummary),
      internalDonorCount: internalRes.count ?? 0
    });
  }
  ...
  async function loadCampaign(nextCampaignId: string) {
    ...
    const rows = await loadMilestoneRows(data.id);
    await refreshSnapshot(data, rows);
  ...
  return (
    <section className="admin-panel">
      ...
      <div className="admin-summary-table-wrap">
        <table className="admin-summary-table admin-campaign-table">
          <thead>
            <tr>
              <th>Title</th>
              <th>Description</th>
          ...
          <tbody>
            {campaignOptions.length ? campaignOptions.map((option) => (
              <tr key={option.id}>
                <td>
                  <strong>{option.title || "Untitled campaign"}</strong>
                  {option.id === campaignId && !draftMode ? <span className="muted-text"> (Selected)</span> : ""}
                </td>
                <td className="admin-campaign-description-cell">
                  {option.summary || "Not set"}
                </td>
                <td className="admin-campaign-url-cell">{option.donateUrl || "Not set"}</td>
                <td>{option.sheetName || "Not set"}</td>
                <td>
                  <span className={`admin-badge ${option.featured ? "featured" : "hidden"}`}>
                    {option.featured ? `Featured #${option.homepageOrder || 1}` : "Hidden"}
                  </span>
                </td>
                <td>
                  <span className={`admin-badge ${option.status === "Active" ? "active" : option.status === "Completed" ? "completed" : "not-achieved"}`}>
                    {option.status}
                  </span>
                </td>
                <td>
              ...
              <tr>
                <td colSpan={7} className="muted-text">No campaigns saved yet.</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <table className="admin-summary-table">
          <thead>
            <tr>
              ...
              <th>Public Raised</th>
              <th>Internal Raised</th>
              <th>Total Raised</th>
              <th>Public Donors</th>
              <th>Internal Donors</th>
              <th>Total Donors</th>
              <th>Progress</th>
              <th>Status</th>
            </tr>
          ...
          <tbody>
            <tr>
              ...
              <td>{formatCurrency(summarySnapshot.publicRaised)}</td>
              <td>{formatCurrency(summarySnapshot.internalRaised)}</td>
              <td>{formatCurrency(summarySnapshot.totalRaised)}</td>
              <td>{summarySnapshot.donorCount}</td>
              <td>{summarySnapshot.internalDonorCount}</td>
              <td>{summarySnapshot.donorCount + summarySnapshot.internalDonorCount}</td>
              <td>
            ...
            <tbody>
              {visibleMilestoneRows.length ? visibleMilestoneRows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>
                    <span className={`admin-badge ${row.status === "Active" ? "active" : row.status === "Completed" ? "completed" : "not-achieved"}`}>
                      {row.status}
                    </span>
                  </td>
                  <td>
                ...
                <tr>
                  <td colSpan={7} className="muted-text">No milestone buckets yet.</td>
                </tr>
              )}
            </tbody>
          </table>
