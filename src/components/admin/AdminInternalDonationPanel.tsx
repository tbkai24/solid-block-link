...
export function AdminInternalDonationPanel() {
  ...
  async function loadEntries(nextCampaignId: string) {
    ...
    try {
      const [entryRows, campaignRes] = await Promise.all([
        fetchInternalDonations(nextCampaignId),
        supabase
          ?.from("campaigns")
          .select("internal_amount")
      ...
      setTotal(Number(campaignRes?.data?.internal_amount ?? entryRows.reduce((sum, item) => sum + Number(item.amount ?? 0), 0)));
    } finally {
  ...
  async function handleCampaignChange(nextCampaignId: string) {
    ...
    try {
      await Promise.all([loadMilestones(nextCampaignId), loadEntries(nextCampaignId)]);
    } catch (error) {
      showAlert("danger", error instanceof Error ? error.message : "Failed to load selected campaign donations.");
    }
  }
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const options = await loadCampaignOptions(campaignRes.data?.id ?? "");
        const nextCampaignId = campaignRes.data?.id ?? options[0]?.id ?? "";
          ...
          <tbody>
            {entries.map((item) => (
              <tr key={item.id}>
                ...
                <td>
                  <span className={`admin-badge ${item.milestone_id ? "primary" : "neutral"}`}>
                    {item.milestone_id ? (milestoneLookup[item.milestone_id] ?? "Tagged milestone") : "Not tagged"}
                  </span>
                </td>
                <td><strong>{item.name}</strong></td>
                <td><strong>{formatCurrency(item.amount)}</strong></td>
                <td className="admin-campaign-description-cell">{item.notes || "No notes"}</td>
                ...
                <td>
                  <div className="admin-table-actions">
                    ...
                    <button className="admin-inline-button danger" type="button" onClick={() => void handleDelete(item.id, item.amount)}>
                      Delete
                    </button>
                  </div>
                </td>
            ...
            {!entries.length && !loading ? (
              <tr>
                <td colSpan={7} className="muted-text">{loadingEntries ? "Loading internal donations..." : "No internal donations yet."}</td>
              </tr>
            ) : null}
          </tbody>
        </table>
