...
import { FiArrowRight, FiShield, FiTarget, FiTrendingUp, FiUsers } from "react-icons/fi";
...
export function ProgressSection({ progress, donateCta, campaignTitle, milestone, donateHref }: ProgressSectionProps) {
  ...
  const stats = [
    ...
    {
      label: "Donors",
      value: totalDonorCount.toString(),
      note: (
        <div className="donor-breakdown-row">
          <span className="donor-pill"><FiUsers aria-hidden="true" /> Public {progress.donorCount}</span>
          <span className="donor-pill"><FiShield aria-hidden="true" /> Internal {internalDonorCount}</span>
        </div>
      ),
      icon: <FiUsers />
    },
    { label: "Progress", value: `${formatPercentage(progress.percent)}%`, icon: <FiTrendingUp /> }
  ];
      ...
      <div className="stats-grid compact">
        {stats.map((stat) => (
          <article className="stat-card" key={stat.label}>
            ...
            {"note" in stat ? <div className="stat-card-note">{stat.note}</div> : null}
          </article>
        ))}
      </div>
    </section>
